...
const Navbar = () => {
  ...
  return (
    <nav className="bg-dark-text text-white shadow-md fixed w-full top-0 z-50">
        ...
        <div className="flex justify-between items-center h-16">
          ...
          <div className="hidden md:block">
            <div className="ml-10 flex items-baseline space-x-4">
              <LanguageLink
                to="/carta"
                className={`px-3 py-2 rounded-md text-base text-cream font-medium transition-colors ${
          ...
          >
            <button
              ...
              className="flex items-center gap-3 bg-terracotta/10 border-2 border-cream/30 text-cream hover:border-cream hover:shadow-lg px-4 py-2.5 rounded-xl text-sm font-medium transition-all duration-300 group backdrop-blur-sm"
            ...
            {isLanguageOpen && (
              <div className="absolute top-full right-0 mt-2 w-64 bg-dark-text rounded-xl shadow-2xl border border-cream/20 overflow-hidden z-50 transform transition-all duration-200 origin-top-right animate-in fade-in slide-in-from-top-2">
                <div className="p-1">
                  {languages.map((language) => (
                    <button
                      ...
                      className={`w-full flex items-center gap-3 px-4 py-3 rounded-lg text-left transition-all duration-200 group ${
                        language.code === currentLang
                          ? "bg-terracotta/10 text-cream border border-cream/20"
                          : "hover:bg-terracotta/5 text-cream hover:text-terracotta"
                      }`}
                    ...
                    >
                      ...
                      {language.code === currentLang && (
                        <Icons.Check className="w-4 h-4 text-cream" />
                      )}
                    </button>
                  ))}
                </div>
      ...
      {isMenuOpen && (
        <div className="md:hidden border-t border-cream">
          <div className="px-2 pt-2 pb-3 space-y-1 sm:px-3 bg-dark-text shadow-lg">
            <LanguageLink
              to="/carta"
              onClick={() => setIsMenuOpen(false)}
              className={`block px-3 py-2 rounded-md text-base text-cream font-medium w-full text-left transition-colors ${
                getCurrentBasePath() === "/carta"
                  ? "bg-terracotta/20 text-cream"
                  : "hover:bg-terracotta/10 hover:text-terracotta"
              }`}
